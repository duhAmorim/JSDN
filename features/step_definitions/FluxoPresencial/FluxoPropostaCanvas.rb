...
Dado('que cadastro uma empresa via API no ambiente ambienteAPI') do 
  read_config_yaml()
  ...
  puts "Ambiente de teste: #{@ambiente}"
  @fluxoPresencialCPF = @cpf
  @fluxoPresencialCNPJ = @cnpj
  @fluxoPresencialEmpresa = @empresa


  #Escreve no arquivo de variáveis globais com o novo CPF, CNPJ, EMAIL criado no CANVAS
  require 'yaml'
  config = YAML.load_file('configuracoesGlobaisTeste.yaml')
  config['fluxoPresencialCPF'] = "#{@fluxoPresencialCPF}"
  config['fluxoPresencialCNPJ'] = "#{@fluxoPresencialCNPJ}"
  config['fluxoPresencialEmail'] = "#{@fluxoPresencialEmail}"
  config['fluxoPresencialEmpresa'] = "#{@fluxoPresencialEmpresa}"
...
Quando('visualizar a pagina principal via Canvas posso informar usuario e senha')  do 
 adm.usuario.set @usuarioFluxoPresencial
 adm.senha.set @senhaFluxoPresencial
 adm.btnEntrar.click
 tirar_foto('fluxoDescontoCanvasBtnEntrar', 'passou')
 iniAdm.btnGerenciar.click 
 tirar_foto('fluxoDescontoCanvasBtnGerenciar', 'passou')
end

Quando('realizar a compra de um produto inserindo o nome da oferta e nome do produto') do 
  page.driver.browser.switch_to().window(page.driver.browser.window_handles.last)
  menu.menuCatalogo.click
  tirar_foto('fluxoDescontoCanvasMenuCatalogo', 'passou')
  cat.textoPesquisa.set @nomeOferta
  cat.btPesquisarProduto.click
  tirar_foto('fluxoDescontoCanvasBtnPesquisarProduto', 'passou')
  cat.mouseHover(@nomeOferta)
  cat.verOferta.click
  tirar_foto('fluxoDescontoCanvasBtnVerOferta', 'passou')
sleep 5

#Condicional se a pagina possui o botão "proximo" no carroussel de produtos
  if page.has_css? cat.btnCarouselLateral, visible: true
    until page.has_css? cat.clickNomeOfertaCarousel(@nomeProduto), visible: true do
        cat.btnSetaCarousel.click
          break if
            page.has_css? cat.clickNomeOfertaCarousel(@nomeProduto)
    end
  end
  cat.clickNomeOfertaCarouselDealer(@nomeProduto)
  car.btnCriarProposta.click
  tirar_foto('fluxoDescontoCanvasBtnCriarProposta', 'passou')
end

Então('enviar a proposta para o cliente') do
  car.btnEnviarProposta.click
  tirar_foto('fluxoDescontoCanvasBtnEnviarProposta', 'passou')
end
